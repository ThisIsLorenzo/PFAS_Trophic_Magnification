...
Vaidation of the model used for subgroup meta-regression analysis.
...
## mod_chem1
```{r, eval=FALSE}
mod_chem1 <- rma.mv(ln_TMF ~ PFAS_ID - 1,
                   V = VCV_chem,
...
# Save the model
save(mod_chem1, file = here("Rmodels", "mod_chem1.RData"))
```

```{r, eval=FALSE}
load(here("Rmodels", "mod_chem1.RData"))
summary(mod_chem1)
...
for (i in parameter_indices) {
  ...
  dat_ll <- profile(mod_chem1, tau2 = i)
...
mod_chem1_profile_results <- do.call(rbind, profile_results)

# Save the table
write.csv(mod_chem1_profile_results, here("Rdata/mod_chem1_profile_results.csv"), row.names = FALSE)
```
...
summary(mod_chem3)
# Identify the indices of the variance components for the parameters with klvl > 10
parameter_indices3 <- c(17, 47, 32, 51, 42, 33, 50, 20, 40, 34, 49, 46, 30, 36, 48, 31, 38)

# Initialize a list to store the results for each parameter
profile_results3 <- list()

# Loop through each index
for (i in parameter_indices3) {
  # Get the profile data for the current variance component
  dat_ll <- profile(mod_chem3, tau2 = i)
...
# Combine all results into a single data frame
mod_chem3_profile_results <- do.call(rbind, profile_results3)

# Save the table
write.csv(mod_chem3_profile_results, here("Rdata/mod_chem3_profile_results.csv"), row.names = FALSE)
```

# Model validation

To validate the models, we are going to run mod_chem3 on a subset dataset for each PFAS chemical which has more than 10 effect sizes and then extract the estimate and sum(sigma-squared) values.
...
## mod_chem1
```{r}
mod_chem1_profile_results <- read.csv(here("Rdata/mod_chem1_profile_results.csv"))
...
mod_chem1_profile_results <- mod_chem1_profile_results %>%
  mutate(parameter_index = param_names[as.character(parameter_index)])

mod_chem1_profile_results <- mod_chem1_profile_results %>%
  left_join(PFAS_sigma2, by = c("parameter_index" = "Chemical"))

ggplot(mod_chem1_profile_results, aes(x = tau2, y = log_likelihood, color = parameter_index)) +
  ...
  labs(
    title = "mod_chem1 Likelihood Profile",
    x = expression(tau^2),
    y = "Log-likelihood",
...
ggsave(here("figs/supplements","mod_chem1_likelihoodprofile.png"),
       width = 10,
       height = 8)
```
...
```{r, eval=FALSE}
mod_chem3_profile_results <- read.csv(here("Rdata/mod_chem3_profile_results.csv"))

mod_chem3_profile_results <- mod_chem3_profile_results %>%
  mutate(parameter_index = param_names[as.character(parameter_index)])

ggplot(mod_chem3_profile_results, aes(x = tau2, y = log_likelihood, color = parameter_index)) +
  geom_line() +  # Add lines to connect points for each parameter_index
  geom_point(size=1) + # Add points to show individual data points
  facet_wrap(~ parameter_index, scales = "free") + # Separate plots for each parameter_index
  geom_vline(aes(xintercept = Sigma2), color = "red", linetype = "dashed") +
  labs(
    title = "mod_chem3 Likelihood Profile",
    x = expression(tau^2),
    y = "Log-likelihood",
    color = "Parameter Index"
  ) +
